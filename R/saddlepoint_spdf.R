...
## generate allele frequencies for a population of size n, with L loci and
## k allele types at each locus, generating frequencies from beta distributions
gen.beta.freq.loci = function(L = 10, n = 100, k = 5, alpha=0.5, beta=0.7)
{
    # generate the raw probabilities of each allele type
    p <- matrix(rbeta(L*k, shape1=alpha,shape2=beta), nrow=k, ncol=L)

    # normalize for each locus
    ptotal <- colSums(p)
    pnorm <- p/matrix(rep(ptotal, times=k), nrow=k, byrow=TRUE)

    # generate allele frequencies
    plist <- lapply(seq_len(ncol(pnorm)), function(i) pnorm[,i])

    have.zero.frequencies <- TRUE
    x <- mapply(rmultinom, rep(1,times=L), rep(n,times=L), plist)
    while (have.zero.frequencies)
    {
        problem.idxs <- which(apply(x==0,2,any))
        if (length(problem.idxs) == 0) have.zero.frequencies <- FALSE
        else x[,problem.idxs] <- mapply(rmultinom, rep(1,times=length(problem.idxs)),
                                        rep(n,times=length(problem.idxs)),
                                        plist[problem.idxs])
    }
...
## generate allele frequencies for a population of size n, with L loci and
## k allele types at each locus, generating frequencies from beta distributions
gen.beta.freq.varying.num.alleles = function(L = 10, n = 100, k = 5, alpha=0.5, beta=0.7)
{
    # generate the raw probabilities of each allele type
    p <- matrix(rbeta(L*k, shape1=alpha,shape2=beta), nrow=k, ncol=L)

    # reset some of the raw allele frequencies to zero to reduce the number of
    # alleles at some loci
    # but ensure there are at least 2 alleles left at each locus
    if (k > 2)
    {
        num_to_delete <- rbinom(n=L, size=k-2, prob=1/(k))
    }
    else
    {
        num_to_delete <- rep(0, times=L)
    }

    x <- list()
    for (idx in 1:L)
    {
        if (num_to_delete[idx] > 0)
        {
            idxs_to_delete <- sample(1:k, num_to_delete[idx], replace=FALSE)
            p_locus <- p[-idxs_to_delete,idx]
        }
        else p_locus <- p[,idx]

        # normalize for each locus
        ptotal <- sum(p_locus)
        pnorm <- p_locus/(rep(ptotal, times=length(p_locus)))

        # generate allele frequencies
        have.zero.frequencies <- TRUE
        x_locus <- rmultinom(1, n, pnorm)
        while (have.zero.frequencies)
        {
            problem.idxs <- which(x_locus==0)
            if (!any(x_locus == 0)) have.zero.frequencies <- FALSE
            else x_locus <- rmultinom(1, n, pnorm)
        }

        x[[idx]] <- as.vector(x_locus)
    }

    x
}

## generate allele frequencies for a population of size n-k, with L loci and
## k allele types at each locus, allele frequencies generated from
## multinomial distribution with equal probabilities
gen.many.loci = function(L = 1000, n = 100, k = 2)
{
    have.zero.frequencies <- TRUE
    x <- rmultinom(L, n, rep(1/k, times=k))
    while (have.zero.frequencies)
    {
        problem.idxs <- which(apply(x==0,2,any))
        if (length(problem.idxs) == 0) have.zero.frequencies <- FALSE
        else x[,problem.idxs] <- rmultinom(length(problem.idxs), n, rep(1/k, times=k))
    }
...
## generate allele frequencies for a population of size n, with L loci and
## 2 allele types at each locus, where one allele type is far more common
## than the other
## if a locus ends up with only one allele type, reset it to have 1 of the
## rare allele type and n-1 of the common allele type i.e. no locus is fixed
gen.many.loci.unbalanced = function(L = 1000, n = 100)
{
    x <- rmultinom(L, n, c(5/n, 1-5/n))
    problem.idxs <- which(x[1,] == 0)
    x[1,problem.idxs] <- 1
    x[2,problem.idxs] <- n-1
    nutab <- lapply(seq_len(ncol(x)), function(i) x[,i])
    return(nutab)
}

plot.K1 <- function(L=1000, n=10, k=2)
{
    nutab <- gen.many.loci(L=L,n=n,k=k)
    info <- calc.multi.locus.probs.func(nutab)
    multi.K.params <- make.K.params(info$dist)

    one.locus.K1 <- function(Kparams,s) sum(Kparams$r1power.values*(Kparams$probs^(s+1)))/sum(Kparams$r0power.values*(Kparams$probs^(s+1)))

    all.loci.K1 <- function(z) sum(sapply(multi.K.params, one.locus.K1, s=z))

    lower = -50
    upper = 100

    t <- seq(lower,upper,2)
    x <- sapply(t,all.loci.K1)

    if (all(is.nan(x)))
    {
        print('Stopping plot. All x-values are NaNs.\n')
        flush.console()
    }
    else if (any(is.nan(x)))
    {
        print('Some x-values are NaNs.\n')
        flush.console()
        non_nan_idxs <- which(!is.nan(x))
        x <- x[non_nan_idxs]
        t <- t[non_nan_idxs]
    }
...
plot.beta <- function(alpha=1,beta=1)
{
    xvals <- seq(0,1,0.01)
    yvals <- dbeta(xvals, shape1=alpha,shape2=beta)
    plot(xvals,yvals, type="l",
         main=paste0("Beta distribution with alpha=",alpha," and beta=",beta))
}
