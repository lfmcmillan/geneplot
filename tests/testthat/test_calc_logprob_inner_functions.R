...
test_that("calc_logprob_missing runs the correct calculations.", {
    ...
    all_loci_SCDF_qsearch_params <- calc.qsearch.params(posterior_nu_list,c("Pop1","Pop2"),logten=TRUE,leave_one_out=FALSE)

    expect_equal(TRUE,all_loci_SCDF_qsearch_params$Pop1$Fh.min > 0 && all_loci_SCDF_qsearch_params$Pop1$Fh.min < 1)
    expect_equal(TRUE,all_loci_SCDF_qsearch_params$Pop1$Fh.max > 0 && all_loci_SCDF_qsearch_params$Pop1$Fh.max < 1)
    expect_equal(TRUE,all_loci_SCDF_qsearch_params$Pop1$Fh.max > all_loci_SCDF_qsearch_params$Pop1$Fh.min)
    expect_equal(TRUE,all_loci_SCDF_qsearch_params$Pop2$Fh.min > 0 && all_loci_SCDF_qsearch_params$Pop2$Fh.min < 1)
    expect_equal(TRUE,all_loci_SCDF_qsearch_params$Pop2$Fh.max > 0 && all_loci_SCDF_qsearch_params$Pop2$Fh.max < 1)
    expect_equal(TRUE,all_loci_SCDF_qsearch_params$Pop2$Fh.max > all_loci_SCDF_qsearch_params$Pop2$Fh.min)
    ...
    Pop1_post.nu.pop <- lapply(posterior_nu_list["Loc1"], function(x) x["Pop1",]) ## Note: must use "lapply" to preserve correct format
    Pop1_post.info <- calc.multi.locus.probs.func(Pop1_post.nu.pop, leave_one_out=FALSE)
    Pop1_Loc1_dist <- Pop1_post.info$dist$Loc1
    Pop1_multi.K.params <- make.K.params(Pop1_post.info$dist)
    Pop1_mean.pop <- mu(Pop1_multi.K.params, twoPops=FALSE)

    ## Test the mean of the distribution
    expect_equivalent(sum(Pop1_Loc1_dist['values',]*(Pop1_Loc1_dist['probs',])),Pop1_mean.pop)

    Pop1_indiv_p <- Fhat(Pop1Row1_raw, Pop1_post.info, mean.pop=Pop1_mean.pop, logten=TRUE, twoPops=FALSE)

    Pop1_q <- calc.quantiles.uniroot(all_loci_SCDF_qsearch_params, posterior_nu_list,
                                "Pop1", Pop1_indiv_p, logten=TRUE, leave_one_out=FALSE)

    Pop2_post.nu.pop <- lapply(posterior_nu_list["Loc1"], function(x) x["Pop2",]) ## Note: must use "lapply" to preserve correct format
    Pop2_post.info <- calc.multi.locus.probs.func(Pop2_post.nu.pop, leave_one_out=FALSE)
    Pop2_multi.K.params <- make.K.params(Pop2_post.info$dist)
    Pop2_mean.pop <- mu(Pop2_multi.K.params, twoPops=FALSE)
    Pop2_indiv_p <- Fhat(Pop2Row1_raw, Pop2_post.info, mean.pop=Pop2_mean.pop, logten=TRUE, twoPops=FALSE)

    Pop2_q <- calc.quantiles.uniroot(all_loci_SCDF_qsearch_params, posterior_nu_list,
...
test_that("calc.multi.locus.probs.func runs the correct calculations.", {

    nu1 <- 2
    dist1 <- calc.probs.func(nu1,FALSE)
    expect_equivalent(dist1['values',1],0)
    expect_equivalent(dist1['probs',1],1)

    nu2 <- c(1.5,1.5)
    dist2 <- calc.probs.func(nu2,FALSE)
    expect_equivalent(dist2['probs',],c(1.5*2.5/3/4,1.5*2.5/3/4,2*1.5*1.5/3/4))
    expect_equivalent(log(1.5*2.5/3/4),calc.min.beta(nu2))
    expect_equivalent(log(2*1.5*1.5/3/4),calc.max.beta(nu2))

    nu3 <- c(9.5,1.5)
    dist3 <- calc.probs.func(nu3,FALSE)
    expect_equivalent(dist3['probs',],c(1.5*2.5/11/12,2*1.5*9.5/11/12,9.5*10.5/11/12))
    expect_equivalent(log(1.5*2.5/11/12),calc.min.beta(nu3))
    expect_equivalent(log(9.5*10.5/11/12),calc.max.beta(nu3))

    nu4 <- c(10+1/3,1/3,1/3)
    dist4 <- calc.probs.func(nu4,FALSE)
    expect_equivalent(dist4['probs',],c(2*1/3*1/3/11/12,
                                        ...
                                        2*1/3*(10+1/3)/11/12,
                                        (10+1/3)*(11+1/3)/11/12))
    expect_equivalent(log(2*1/3*1/3/11/12),calc.min.beta(nu4))
    expect_equivalent(log((10+1/3)*(11+1/3)/11/12),calc.max.beta(nu4))
